...
### 自动加载打包的模型文件
本项目已实现将 `models` 文件夹中的 STL 模型自动打包到桌面应用中，并在启动时自动加载显示的功能。

#### 开发环境中的模型库
在开发环境中，模型库会自动扫描项目根目录下的 `models` 文件夹，将所有 `.stl` 文件作为可导入模型显示。

#### 打包后的应用模型库
在打包后的桌面应用中，模型库会自动加载打包到应用资源中的模型文件，无需额外的 `models` 文件夹。

#### 如何添加模型到应用中
1. 将您的 `.stl` 模型文件放入项目根目录下的 `models` 文件夹
2. 执行 `npm run electron:build` 命令重新打包应用
3. 打包后的应用将自动包含 `models` 文件夹中的所有 STL 模型
4. 打开软件的模型库时，将自动显示这些模型

#### 技术实现说明
- 打包时，`models` 文件夹会通过 `extraResources` 配置自动包含在 Electron 应用包中
- 在生产环境中，应用会通过 IPC 通信从主进程读取模型文件内容，并转换为 URL 供 Three.js 使用
- 在开发环境中，应用使用 Vite 的 `import.meta.glob` 功能动态扫描 `models` 文件夹
...
#### 📂 模型库
*   将 STL 文件放入 `/models` 文件夹，打包应用后模型会自动加载到模型库中
